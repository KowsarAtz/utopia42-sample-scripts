...
async function main() {
    ...
    failed = [];
    success = 0;
    res = await rxjs.firstValueFrom(UtopiaApi.selectBlocks(Object.keys(res).filter(pos => res[pos]).map(posStr => {
        const items = posStr.replace('(','').replace(')','').split(",");
        return {
            x: Number(items[0]),
            y: Number(items[1]),
            z: Number(items[2])
    ...
    console.log(`Selected ${success} out of ${Object.keys(res).length} blocks.`);
    if (failed.length > 0)
        console.warn("Failed to select block at following positions", failed);
}
